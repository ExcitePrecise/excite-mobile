...
const BronzeRoute = ({ route, token, subscriptionLevel, cycle }) => {
  //
  ...
  const handlePay = async () => {
    setIsSubmit(true)
    const paymentDue = await InitialzePayment('qVaGuabMOm', token)
    setIsSubmit(false)
  ...
  const isCurrentPlan = () => {
    if (subscriptionLevel === 1 && cycle === 'Monthly') return true
    return false
  }
  return (
    <ScrollView style={{ flex: 1, backgroundColor: COLORS.lightGray }}>
      <View style={styles.package}>
        <View style={styles.price}>
          <Text style={{ ...FONTS.h2 }}>NGN 1,000</Text>
        </View>
...
export default connect(mapStateToProps)(BronzeRoute)
