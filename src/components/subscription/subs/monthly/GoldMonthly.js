...
const GoldMonthly = ({ route, token, subscriptionLevel, cycle }) => {
  //
  ...
  const handlePay = async () => {
    setIsSubmit(true)
    const paymentDue = await InitialzePayment('qVaGuagMOm', token)
    setIsSubmit(false)
  ...
  const isCurrentPlan = () => {
    if (subscriptionLevel === 3 && cycle === 'Monthly') return true
    return false
  }
  return (
    <ScrollView style={{ flex: 1, backgroundColor: COLORS.lightGray }}>
      <View style={styles.package}>
        <View style={styles.price}>
          <Text style={{ ...FONTS.h2 }}>NGN 10,000</Text>
        </View>
...
export default connect(mapStateToProps)(GoldMonthly)
