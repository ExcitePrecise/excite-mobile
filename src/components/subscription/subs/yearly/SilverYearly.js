...
const SilverYearly = ({ route, token, subscriptionLevel, cycle }) => {
  //
  ...
  const handlePay = async () => {
    setIsSubmit(true)
    const paymentDue = await InitialzePayment('KivJxT6SyO', token)
    setIsSubmit(false)
  ...
  const isCurrentPlan = () => {
    if (subscriptionLevel === 2 && cycle === 'Yearly') return true
    return false
  }
  return (
    <ScrollView style={{ flex: 1, backgroundColor: COLORS.lightGray }}>
      <View style={styles.package}>
        <View style={styles.price}>
          <Text style={{ ...FONTS.h2 }}>NGN 50,000</Text>
        </View>
...
export default connect(mapStateToProps)(SilverYearly)
